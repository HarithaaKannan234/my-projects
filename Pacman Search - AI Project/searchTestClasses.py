...
class PacmanSearchTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if not correct[1]:  # path correct, but wrong expanded nodes
            grades.addMessage('FAIL: %s' % self.path)
            grades.addMessage(
                'Incorrect number of nodes expanded; are you expanding nodes twice, or using the wrong algorithm?')
...
### A test to get True/False or list of True/False reflection answers
### Introduced in AI24 P1 for asking conceptual/reflection questions about search
class BasicQuestionTest(testClasses.TestCase):

    def __init__(self, question, testDict):
        super(BasicQuestionTest, self).__init__(question, testDict)
        self.functionName = testDict['function']

  
        
    def execute(self, grades, moduleDict, solutionDict):
        studentSolution = getattr(moduleDict['reflection'], self.functionName)()

        if ',' in solutionDict['solution']:
            solutionList = solutionDict['solution'][1:-1].split(',')
            self.solution = [str_to_bool(x) for x in solutionList]
        else:
            self.solution = str_to_bool(solutionDict['solution'])
            
        if studentSolution == self.solution:
...
def str_to_bool(s):
    if s == 'True':
        ...
        raise ValueError("Unexpected string value in testcase. Cannot convert to boolean.")
